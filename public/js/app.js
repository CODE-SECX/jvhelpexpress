...
async function loadActivitiesContent(language = "en") {
    try {
        const response = await fetch(
            `/api/activities-content?lang=${language}`,
        );
        const result = await response.json();
...
function updateActivitiesContent(activities) {
    activities.forEach((activity, index) => {
        const activityCard = document.querySelector(
            `.activity-card:nth-child(${index + 1})`,
        );
        if (activityCard) {
            const titleElement = activityCard.querySelector(".activity-title");
            const descElement = activityCard.querySelector(
...
function updateAllContentSections(language) {
    ...
    const existingTranslations = {
        hi: {
            "section-title":
                "श्री जयवीर जीवदया ग्रुप द्वारा परोपकारी गतिविधियां",
        ...
        gu: {
            "section-title":
                "શ્રી જયવીર જીવદયા ગ્રુપ દ્વારા પરોપકારી પ્રવૃત્તિઓ",
...
const activityAnimations = {
    ...
    "diwali-icon":
        "https://assets9.lottiefiles.com/packages/lf20_m9zragkd.json",
    "water-icon":
        "https://lottie.host/4c48890e-e58e-4ec1-9268-996a62ab772a/8ATjLwiT15.lottie",
    "birdhouse-icon":
        "https://assets6.lottiefiles.com/packages/lf20_syqnfe7c.json",
    "feeding-icon":
        "https://assets3.lottiefiles.com/packages/lf20_wdfng7n6.json",
    "education-icon":
        "https://assets6.lottiefiles.com/packages/lf20_w51pcehl.json",
    "rescue-icon":
        "https://assets9.lottiefiles.com/packages/lf20_pbkjxppc.json",
    "year-round-icon":
        "https://assets5.lottiefiles.com/packages/lf20_qpsh9lay.json",
};
...
async function loadActivitiesGallery(language = "en") {
    try {
        const response = await fetch(
            `/api/activities-gallery?lang=${language}`,
        );
        const result = await response.json();
...
function applyFiltersAndSort() {
    ...
    filteredGalleryItems = allGalleryItems.filter((item) => {
        ...
        const matchesFilter =
            currentFilter === "all" || item.category_en === filterCategory;
        const matchesSearch =
            searchQuery === "" ||
...
function changePage(page) {
    if (
        page < 1 ||
        page > Math.ceil(filteredGalleryItems.length / itemsPerPage)
    )
        return;
...
window.prevModalSlide = function () {
    const indicators = document.querySelectorAll(".gallery-modal-carousel-dot");
    currentModalSlide =
        currentModalSlide > 0 ? currentModalSlide - 1 : indicators.length - 1;
    updateEnhancedModalCarousel();
};
            ...
            case "bird-letter":
                if (action === "read-letter") {
                    ...
                    const letterContent = document.querySelector(
                        `[data-animal="bird-letter"] .letter-content`,
                    );
    ...
    createSlides() {
        if (!this.data.length) {
            this.slider.innerHTML =
                '<div class="slide"><div class="slide-content"><p>No activities available</p></div></div>';
            return;
        }

        this.slider.innerHTML = this.data
            .map((item) => {
...
function openGalleryModal(itemId) {
    ...
    stats.innerHTML = Object.entries(activityStats)
        .map(
            ([key, value]) =>
                `<div class="stat-item">
...
window.prevCarouselSlide = function () {
    ...
    window.currentCarouselSlide =
        window.currentCarouselSlide > 0
            ? window.currentCarouselSlide - 1
            : slides.length - 1;
    updateCarouselPosition();
};
...
window.nextCarouselSlide = function () {
    const slides = document.querySelectorAll(".gallery-modal-carousel-slide");
    window.currentCarouselSlide =
        (window.currentCarouselSlide + 1) % slides.length;
    updateCarouselPosition();
};
...
function startCarouselAutoScroll(totalSlides) {
    ...
    window.carouselInterval = setInterval(() => {
        window.currentCarouselSlide =
            (window.currentCarouselSlide + 1) % totalSlides;
        updateCarouselPosition();
    }, 4000);
}
...
function handleWaterDrop() {
    waterDropsFound++;
    const speechBubble = document.querySelector(
        `[data-animal="water"] .speech-bubble p`,
    );
    const progressBar = document.querySelector(
        `[data-animal="water"] .happiness-fill`,
    );
    ...
    if (waterDropsFound === 1) {
        ...
        speechBubble.textContent =
            "Great! You found another way! Just one more to go!";
...
function checkAllBadges() {
    const earnedBadges = Object.values(badges).filter(
        (earned) => earned,
    ).length;
    if (earnedBadges === 4) {
        document
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function initTranslatableElements() {
        const elementsToTranslate = [
            ...
            {
                selector: ".activities-section .section-title",
                key: "section-title",
            },
            {
                selector: ".activity-card:nth-child(1) .activity-title",
                key: "winter-care",
            },
            {
                selector: ".activity-card:nth-child(1) .activity-description",
                key: "winter-desc",
            },
            {
                selector: ".activity-card:nth-child(2) .activity-title",
                key: "festival-joy",
            },
            {
                selector: ".activity-card:nth-child(2) .activity-description",
                key: "festival-desc",
            },
            {
                selector: ".activity-card:nth-child(3) .activity-title",
                key: "animal-welfare",
            },
            {
                selector: ".activity-card:nth-child(3) .activity-description",
                key: "animal-desc",
            },
            {
                selector: ".activity-card:nth-child(4) .activity-title",
                key: "wildlife-conservation",
            },
            {
                selector: ".activity-card:nth-child(4) .activity-description",
                key: "wildlife-desc",
            },
            {
                selector: ".activity-card:nth-child(5) .activity-title",
                key: "daily-feeding",
            },
            {
                selector: ".activity-card:nth-child(5) .activity-description",
                key: "daily-desc",
            },
            {
                selector: ".activity-card:nth-child(6) .activity-title",
                key: "educational-support",
            },
            {
                selector: ".activity-card:nth-child(6) .activity-description",
                key: "educational-desc",
            },
            {
                selector: ".activity-card:nth-child(7) .activity-title",
                key: "animal-rescue",
            },
            {
                selector: ".activity-card:nth-child(7) .activity-description",
                key: "animal-rescue-desc",
            },
            {
                selector: ".activity-card:nth-child(8) .activity-title",
                key: "year-round-support",
            },
            {
                selector: ".activity-card:nth-child(8) .activity-description",
                key: "year-round-desc",
            },
        ];
    ...
    form.addEventListener("submit", async function (e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showFormMessage(
                    result.error ||
                        "Failed to submit your thoughts. Please try again.",
                    "error",
                );
            }
        } catch (error) {
...
function displayRecentThoughts(thoughts) {
    ...
    if (thoughts.length === 0) {
        thoughtsGrid.innerHTML =
            '<p style="text-align: center; color: #666; grid-column: 1/-1;">No thoughts shared yet. Be the first to share your experience!</p>';
        return;
    }

    thoughtsGrid.innerHTML = thoughts
        .map((thought) => {
